...
import AdsEnquiryTableColumn from "./AdsEnquiryTableColumn";
...
const AllAdsEnquiry = () => {
  ...
  const enquiryAdsStore = useSelector((state) => state.enquiry);
  const contactAdsEnquiryList = enquiryAdsStore.contactAdsEnquiryList;
  const [filteredList, setFilteredList] = useState(contactAdsEnquiryList || []);
  ...
  useEffect(() => {
    setFilteredList(contactAdsEnquiryList);
  ...
  const deletePayload = {
    contactType: "Advertisement Enquiry",
    ids: selectedRows,
  };
  ...
  const handleBulkDelete = async (event) => {
    ...
    try {
      ...
      await dispatch(BulkAdsEnqueriesDelete(deletePayload)).unwrap();
      await dispatch(getAdsEnquiryList(payload)).unwrap();
      setOpenDialog(false);
      setSelectedRows([]);
    } catch (error) {
  ...
  const fetchData = async (e) => {
    ...
    setLodingT(true);
    try {
      await dispatch(getAdsEnquiryList(payload));
    } catch {
  ...
  useEffect(() => {
    setTotalCount(enquiryAdsStore?.paginationModel?.totalItems || 0);
  }, [enquiryAdsStore.paginationModel]);
...
export default memo(AllAdsEnquiry);
