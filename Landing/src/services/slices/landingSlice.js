...
export const getLandingSubCategoryDetails = createAsyncThunk(
...
export const getLandingCategoryDetailsByParmalink = createAsyncThunk(
  "getLandingCategoryDetailsByParmalink",
  async (link, thunkAPI) => {
    try {
      const response = await api.get(
        `/landing/category/getBySlug?permalink=${link}`
      );
      return response.data;
    } catch (error) {
      return thunkAPI.rejectWithValue(
...
export const getLandingTagDetailsByParmalink = createAsyncThunk(
  "getLandingTagDetailsByParmalink",
  async (link, thunkAPI) => {
    try {
      const response = await api.get(
        `/landing/posts/getByTag?tag=${link}&perPage=10&currentPage=1`
      );
      return response.data;
    } catch (error) {
      return thunkAPI.rejectWithValue(
...
export const getByCategoryHome = createAsyncThunk(
  "getByCategoryHome",
  async ({ id, limit }, thunkAPI) => {
    try {
      const response = await api.get(
        `/landing/posts/home/getByCategory/${id}?limit=${limit}&page=1`
      );
      return response.data;
    } catch (error) {
      return thunkAPI.rejectWithValue(
...
const landingSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(getLandingSubCategoryDetails.pending, (state) => {
      state.loading = "pending";
    });
    builder.addCase(getLandingSubCategoryDetails.fulfilled, (state, action) => {
      state.loading = "complete_success";
      const categoryId = action.meta.arg;
      state.landingSubCategoryDetails[categoryId] = action.payload?.data || [];
    });
    builder.addCase(getLandingSubCategoryDetails.rejected, (state) => {
      state.authLoading = "complete_failure";
    });

    builder.addCase(getLandingCategoryDetailsByParmalink.pending, (state) => {
      state.loading = "pending";
    });
    builder.addCase(
      getLandingCategoryDetailsByParmalink.fulfilled,
      (state, action) => {
        state.loading = "complete_success";
        state.landingCategoryDetailsByParmalink = action?.payload?.data;
      }
    );
    builder.addCase(getLandingCategoryDetailsByParmalink.rejected, (state) => {
      state.authLoading = "complete_failure";
    });

    builder.addCase(getLandingTagDetailsByParmalink.pending, (state) => {
      state.loading = "pending";
    });
    builder.addCase(
      getLandingTagDetailsByParmalink.fulfilled,
      (state, action) => {
        state.loading = "complete_success";
        state.landingTagDetailsByParmalink = action?.payload?.posts;
      }
    );
    builder.addCase(getLandingTagDetailsByParmalink.rejected, (state) => {
      state.authLoading = "complete_failure";
    });
    ...
    builder.addCase(getLandingCategoryIdMore.fulfilled, (state, action) => {
      ...
      state.landingCategoryIdMoreCount = action?.payload?.totalPosts;
    });
    ...
    builder.addCase(landingRemoveSaveItemList.pending, (state) => {
      state.loading = "pending";
    });
    builder.addCase(landingRemoveSaveItemList.fulfilled, (state, action) => {
      state.loading = "complete_success";
      state.landingSaveItemList = state.landingSaveItemList.filter(
        (item) => item.post.id !== action.meta.arg
      );
    });
    builder.addCase(landingRemoveSaveItemList.rejected, (state) => {
      state.authLoading = "complete_failure";
    });
    ...
    builder.addCase(landingNotificationRead.fulfilled, (state, action) => {
      state.loading = "complete_success";
      state.LandingNotificationListAll =
        state.LandingNotificationListAll.filter(
    ...
    builder.addCase(getCategoryHome.pending, (state) => {
      state.loading = "pending";
    });
    builder.addCase(getCategoryHome.fulfilled, (state, action) => {
      state.loading = "complete_success";
      state.landingCategoryHome = action?.payload?.data;
    });
    builder.addCase(getCategoryHome.rejected, (state) => {
      state.authLoading = "complete_failure";
    });

    builder.addCase(getByCategoryHome.pending, (state) => {
      state.loading = "pending";
    });
    builder.addCase(getByCategoryHome.fulfilled, (state, action) => {
      state.loading = "complete_success";
      // state.landingCategoryByHome = action?.payload?.data;
      const { id } = action.meta.arg;
      if (!state.landingCategoryHome) return;

      const category = state.landingCategoryHome.find(
        (item) => item.index === 0
      );
      if (category && id === category.id) {
        state.internationalNews = action?.payload?.data;
      }

      const nationalCategory = state.landingCategoryHome.find(
        (item) => item.index === 1
      );
      if (nationalCategory && id === nationalCategory.id) {
        state.nationalNews = action?.payload?.data;
      }

      const featuredCategory = state.landingCategoryHome.find(
        (item) => item.index === 2
      );
      if (featuredCategory && id === featuredCategory.id) {
        state.featuredNews = action?.payload?.data;
      }

      const sortCategory = state.landingCategoryHome.find(
        (item) => item.index === 3
      );
      if (sortCategory && id === sortCategory.id) {
        state.sortNews = action?.payload?.data;
      }
      // if (id === 1) {
      //   state.internationalNews = action?.payload?.data;
      // } else if (id === 2) {
    ...
    builder.addCase(getByCategoryHome.rejected, (state) => {
      state.authLoading = "complete_failure";
    });
  },
});
